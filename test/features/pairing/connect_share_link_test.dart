...
class _FakePairingRepository implements PairingRepository {
  _FakePairingRepository(this.code);
  ...
  Future<Result<PairingCode>> host() async {
    hostCalls++;
    final pc = PairingCode.fromTtl(
      value: code,
      ttl: const Duration(minutes: 5),
    );
    _controller.add(PairingState.hosting(pc));
...
void main() {
  ...
  setUp(() async {
    ...
    repo = _FakePairingRepository('424242');
    getIt
  ...
  testWidgets('sharing builds the invite link for the live code and tags the '
      'session shareLink (FR-001/002/017)', (tester) async {
    ...
    await tester.pump(); // build ConnectPage → host()
    await tester.pump(); // hosting state emitted → hosting panel renders

    // The live code is shown and host() ran exactly once (FR-003).
    expect(repo.hostCalls, 1);

    // Tap "Share invite link" → the system share sheet receives the invite link.
    await tester.tap(find.text('Share invite link'));
    await tester.pump();

    expect(sharedTexts, hasLength(1));
    expect(sharedTexts.single, contains(ConnectLink.build('424242')));
    expect(repo.hostCalls, 1); // sharing did NOT regenerate the code (FR-003)

    // When the peer connects, the session is recorded as shareLink (FR-017).
