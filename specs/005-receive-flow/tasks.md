...
- [x] T001 Add `path_provider: ^2.1.6`, `share_plus: ^13.1.0`, `open_filex: ^4.7.0` to `dependencies` in `pubspec.yaml` (verified pub.dev 2026-06-25, Constitution XV); run `flutter pub get`; confirm `pubspec.lock` updated; note `share_plus` 13.x Android toolchain (Java 17 / AGP ≥ 8.12.1) as a deferred-device-build item, no pod churn on the document path
