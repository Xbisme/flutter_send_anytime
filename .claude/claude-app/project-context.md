...
> Last updated: 2026-06-29 (Specs #001–#013 merged via PRs #1–#13; **#013 In-App Viewers merged** — received files now open in in-app image/media/PDF/text viewers + real video thumbnails. **All v1.0 feature specs done** — only the **#014 Polish & Release** sweep remains. **Next: #014 Polish & v1.0 Release**.)
...
- **Latest**: **Spec #012 Home Screen Completion** ✅ **MERGED** (PR #12, merge commit `f899caf`) on branch `012-home-completion` (2026-06-29). The Home tab's placeholder hero/stats/recent-media is replaced with **real data from the #006 transfer-history store only** (no media library, no new permission — FR-016); `HomeCubit` is reactive (live-updates, FR-011) via a pure `HomeDashboardBuilder` + `WatchHomeDashboardUseCase`. Recent photos show real `Image.file` thumbnails (bounded `cacheWidth`) with type-icon fallback; videos = tile (frame deferred to #013). New full-screen **"Xem tất cả"** (`AppRoutes.homeSeeAll` + `SeeAllCubit`/`SeeAllPage`, lazy); item tap → existing History detail (#006). Additive + feature-local (+ core-pure `FileCategory`/`MediaThumbnail`); **no new packages, no engine/protocol/schema edits**. `dart analyze lib test` 0 · `flutter test` **318 passed** (33 new) · `dart format` clean. DesignSync verified (component/token-level project; screens follow §Screen 01). Deferred (non-blocking): on-device UI pass (T031; no two-device smoke — local feature). See [changelog.md](changelog.md) + [specs/012-home-completion/](../../specs/012-home-completion/). **Next: #013 In-App Viewers.**
- **Latest**: **Spec #013 In-App File Viewers** ✅ **MERGED** (PR #13, merge commit `e452bc4`) on branch `013-in-app-viewers` (2026-06-29). Tapping a **received** known-type file (History detail, Receive-complete list, Home recent / See-all grids) opens it in an in-app viewer instead of bouncing to the OS sheet: a core viewer-kind resolver splits the #012 `FileCategory` `files` bucket into audio/PDF/text/code → image→`InteractiveViewer` (zoom/pan), video/audio→one shared media player (play/pause/scrubber, stops on dismiss), PDF→PDFium paged viewer, text/code→`SelectableText` (≈1 MB cap → truncated-with-notice). Unsupported types + **sent** files fall back to existing `open_filex`/`share_plus` (no dead end). Real video thumbnails replace the #012 placeholder in Home/See-all media grids (lazy, bounded memory, **on-disk LRU cache** keyed by path+mtime). All on-device (works in airplane mode), reuses on-disk paths (no re-copy), hides bottom nav, respects Reduced-Motion + a11y; additive — **no engine/protocol/schema edits**. **`dart analyze lib test` = 0 · `flutter test` = 357 passed (39 new) · `dart format` clean.** Packages (verified pub.dev): `video_player` ^2.11.1 + `pdfrx` ^2.4.4 (flutter `>=3.41.0` exact floor) + `video_thumbnail_plus` ^0.0.2 (fork of abandoned `video_thumbnail`); image+text viewers use Flutter built-ins. Native: first `pod install` since #007; 2 device-run DI fixes folded in. Deferred (device): on-device viewer pass on 2 devices. See [changelog.md](changelog.md) + [specs/013-in-app-viewers/](../../specs/013-in-app-viewers/). **Next: #014 Polish & v1.0 Release.**
...
| 012 | Home Screen Completion (real data + See-all) | ✅ **Merged** (PR #12) · device UI pass (T031) deferred | `012-home-completion` |
| 013 | In-App File Viewers | ✅ **Merged** (PR #13) · on-device viewer pass deferred | `013-in-app-viewers` |
| 014 | Polish & v1.0 Release | 🟡 **Next** | `014-polish-v1-release` |
