...
- **Next spec**: **#006 Lịch sử (History)** — drift (SQLite) persistence of transfer records (direction/peer/files/timestamp/status), by-day grouped list + search, re-send, clear; backfills the Home "recent" lists. Hooks `core/` lifecycle so #004/#005 write a record on terminal transfer state.
