...
> Last updated: 2026-06-26 (Specs #001–#009 merged into `main` via PRs #1–#9; **#010 Settings & Preferences ✅ implemented on branch `010-settings`** — 2-device smoke deferred. **Next: #011 Polish & v1.0 Release.**)
...
- **Next spec**: **#010 Settings & Preferences** — device profile (name + avatar shown to peers) + ToggleRow group (Tự động nhận / Lưu vào Thư viện / Thông báo / Giao diện tối light·dark·system) + default download location + signaling-endpoint override/diagnostics + language picker (EN/VI/system) + About/version footer. New packages: `shared_preferences`, `package_info_plus`, `in_app_review`. Run `/speckit.specify` to start. See [`sdd-roadmap.md`](sdd-roadmap.md) §Spec #010.
