...
- **Next spec**: **#005 Receive Flow (Nhận)** ⭐ MVP — receive screen (enter 6-digit code, reusing the role-parameterized Connect hub) → incoming-transfer accept/reject prompt → save to platform location → live Progress/Complete (reused from #004). After #005 the full pair→send→receive loop works.
...
| 005 | Receive Flow (Nhận) ⭐ MVP | ⬜ Not started 🟡 Next | `005-receive-flow` |
